...
export default function DeleteControls({
  ...
  const { pagesToDelete, errors: localErrors } = pages.trim()
    ? parsePages(pages, totalCount)
    : { pagesToDelete: new Set<number>(), errors: [] };

  const validCount = pagesToDelete.size;
  const hasErrors = localErrors.length > 0;
  const isSubmitDisabled = isDeleting || hasErrors || !pages.trim();
    ...
    <div className="mt-8 flex flex-col gap-6 rounded-3xl border border-gray-800 bg-black/25 p-6 text-left max-w-xl mx-auto">
      ...
      <div className="space-y-1">
        ...
        <p className="text-sm text-gray-400">
          Enter page numbers or ranges to exclude from the document.
        </p>
      </div>
      ...
      <div className="space-y-3">
        ...
        <input
          ...
          className={`w-full rounded-xl border bg-black/40 px-4 py-3.5 text-white placeholder-gray-600 outline-none transition duration-300 ${
            hasErrors
              ...
              : "border-gray-800 focus:border-rose-500/70"
          }`}
        />
        ...
        <div className="rounded-xl bg-white/5 p-3 text-xs text-gray-400 space-y-1">
          ...
          <ul className="list-disc pl-4 space-y-0.5">
            <li><code className="text-rose-300 font-semibold">1</code> — deletes the first page only</li>
            <li><code className="text-rose-300 font-semibold">1,3,5</code> — deletes pages 1, 3, and 5</li>
            <li><code className="text-rose-300 font-semibold">4-10</code> — deletes pages 4 through 10 inclusive</li>
            <li><code className="text-rose-300 font-semibold">1,5-8,11</code> — deletes page 1, pages 5 to 8, and page 11</li>
          </ul>
        </div>
      </div>
        ...
        <div className="transition-all duration-300">
          {hasErrors ? (
            ...
            <div className="flex items-center gap-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 p-3 text-emerald-300 text-xs">
              ...
              <p>
                Valid selection. Will delete <strong>{validCount}</strong> page{validCount === 1 ? "" : "s"} (At least {totalCount - validCount} page{totalCount - validCount === 1 ? "" : "s"} will remain).
              </p>
            </div>
          )}
        </div>
      ...
      <div className="mt-2 flex justify-end">
        <Button
          ...
          className="w-full sm:w-auto min-w-[180px] bg-rose-600 hover:bg-rose-700 hover:shadow-[0_10px_30px_rgba(244,63,94,0.25)] text-white"
        >
